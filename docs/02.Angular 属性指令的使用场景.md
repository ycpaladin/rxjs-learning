...
以上面为`HTML`节点添加属性指令为例, 我们先关注一下`nz-input`这个属性指令，翻开源码我们可以看到，它做的绝大部分事情是根据传入的`@Input`属性来控制`input`元素的外观，整个代码100行都不到，非常简单。有兴趣可以去 [看看](https://github.com/NG-ZORRO/ng-zorro-antd/blob/master/components/input/input.directive.ts) ，这里就不深入展开了。

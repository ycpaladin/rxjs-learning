...
`pipe`的字面意思就是管道，他是`Observable`中的方法，`pipe`中传入的参数被称作操作符（**`operator`**），操作符可以有多个，他们决定了数据的流向。

为了方便理解，我打开了很久没玩过的马里奥派对，截了张图，看图的右侧：

![image-20220208160323654](./assets/image-20220208160323654.png)

这个是划船游戏，`起点`在最下方，在划船的过程中你可以选择走`不同的河道（向左划或者向右划）`，在最上方是分别对应着5个不同的终点,。那么这个跟`pipe`有啥毛线关系呢？

其中`起点`可以看作一个可观察对象，`不同的河道`我们可以将其看作操作符（**`operator`**），那么从起点到终点整个划船运动轨迹你可以看作为一个`pipe`。

##### Operator

说到`pipe`就不得不说说操作符了，在划船游戏中，`不同的河道遇到的障碍是不一样的`，但是我每次选择重新开始划，`选择去我之前去过的河道，遇到的障碍都是一样的`。 这又意味着什么呢？

`不同的河道遇到的障碍是不一样的`可理解为每一个操作符的作用是不一样的，在Rxjs中操作符的数量大概是113个。
